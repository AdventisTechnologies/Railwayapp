...
//   - `db`      : every normal CRUD query (login, insert, fetch-for-dashboard)
//   - `watchDb` : ONLY the long-lived change-stream watch (real-time MQTT bridge)
// A change-stream cursor is a long-lived, continuously-read operation on its
// socket. Sharing that same socket with regular request/response queries is
// what was causing "connection closed: socket reset by peer" on ordinary
// inserts (e.g. every Stop) — the change stream and a normal query were
// fighting over the same TCP connection. Splitting them mirrors the old
// Postgres server's conn/listenConn split for the same reason.
late Db db;
late Db watchDb;
late MqttServerClient mqttClient;
...
Future<void> connectDB() async {
  db = await _openConnection();
  print("Connected to MongoDB (database: ${db.databaseName}) — CRUD connection");
  watchDb = await _openConnection();
  print("Connected to MongoDB (database: ${watchDb.databaseName}) — change-stream connection");
}
...
// failing at once) previously meant the "one retry" often landed on a
// connection that hadn't stabilized yet.
Future<T> _withRetry<T>(Future<T> Function() action, {int maxAttempts = 4}) async {
  var attempt = 0;
  while (true) {
    try {
      return await action();
      ...
      if (attempt >= maxAttempts) {
        print("Query failed after $attempt attempts ($e). Giving up.");
        print("  If EVERY request fails like this (not just occasionally), it's not a");
        print("  transient blip — check: (1) Atlas > Network Access has this server's");
        print("  current outbound IP allowed (0.0.0.0/0 while testing), (2) the cluster");
        print("  in the URI isn't paused, (3) the username/password in _mongoUri are");
        print("  correct and haven't been rotated.");
        rethrow;
      }
      print("Query failed ($e). Reconnecting to MongoDB and retrying (attempt $attempt)...");
      await _reconnect();
...
// ==========================================
Future<void> startMongoChangeStreamBridge() async {
  // Uses `watchDb` (its own dedicated connection), never `db` — see the note
  // on the `db`/`watchDb` declaration above for why.
  final collection = watchDb.collection('machine_sensor_data');
  final stream = collection.watch(
    <Map<String, Object>>[],
    changeStreamOptions: ChangeStreamOptions(fullDocument: 'updateLookup'),
  );
  ...
  stream.listen((event) {
    ...
    // If the change stream itself drops (Atlas free-tier cursors get killed
    // periodically), reconnect JUST watchDb and restart the watch — this no
    // longer touches `db`, so ordinary CRUD requests (Start/Stop/dashboard)
    // are completely unaffected by a change-stream hiccup.
    print("[MQTT BRIDGE ERROR] Change stream error: $e — reopening watch connection...");
    try {
      watchDb = await _openConnection();
      await startMongoChangeStreamBridge();
    } catch (e2) {
      print("[MQTT BRIDGE ERROR] Failed to reopen change-stream connection: $e2");
    }
  });
}
